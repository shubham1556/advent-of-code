...
public class Day7Part2 {
    ...
    public static int getJokerCardTypeRank(Map<Character, Integer> cardMap) {
        if (cardMap.size() == 1) {
            return 7;
        }

        Integer jCount = cardMap.get('J');
        cardMap.remove('J');
        ...
        Integer maxCount = cardMap.get(maxKey) + jCount;
        cardMap.put(maxKey, maxCount);

        return getNormalCardTypeRank(cardMap);
    }

    public static int getNormalCardTypeRank(Map<Character, Integer> cardMap) {
        if (cardMap.size() == 1) {
            return 7;
        }
    ...
    static class CardComparator implements java.util.Comparator<Poker> {
        ...
        public int compare(Poker a, Poker b) {
            ...
            int card1TypeRank = card1Map.keySet().contains('J') ? getJokerCardTypeRank(card1Map)
                    : getNormalCardTypeRank(card1Map);
            int card2TypeRank = card2Map.keySet().contains('J') ? getJokerCardTypeRank(card2Map)
                    : getNormalCardTypeRank(card2Map);
