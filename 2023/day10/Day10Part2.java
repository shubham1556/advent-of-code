...
public class Day10Part1 {
    private static final char PATH_PIPE_CHAR = '9';
    ...
    public static void main(String[] args) {
        ...
        while (!pointer1.equals(pointer2)) {
            ...
            pointer1 = next1;
            markPathPipe(maze, pointer1Previous);
            ...
            pointer2 = next2;
            markPathPipe(maze, pointer2Previous);
        }
        markPathPipe(maze, pointer1);
        ...
        for (i = 0; i < maze.size(); i++) {
            ...
            for (int j = 0; j < maze.get(0).size() - 1; j++) {
                if (maze.get(i).get(j) == PATH_PIPE_CHAR) {
                    shouldPaint = !shouldPaint;
                } else if (maze.get(i).get(j) == '.' && shouldPaint) {
                    insideCount ++;
                }
            }
        }

        System.out.println("INSIDE count is" + insideCount);
    }

    static void markPathPipe(List<List<Character>> maze, Coordinate coordinate) {
        maze.get(coordinate.x).set(coordinate.y, PATH_PIPE_CHAR);
    }
